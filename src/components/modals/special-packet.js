...
const Content = React.forwardRef(({ onOpen = () => {}, onFailure = () => {} }, ref) => {
	...
	async function openPackAnimation() {
		...
		let specialStickerId;
		try {
			const { data } = await API.openSpecialPack();
			specialStickerId = data.specialStickerId;
			dispatch(PackActions.openSpecialPack(specialStickerId));
		} catch (e) {
			console.error(e);
			onFailure();
			return;
...
export default function SpecialPacketsModal() {
	...
	function next(event) {
		event.stopPropagation();
		setIsPackOpen(false);
		if (availableSpecialPacks === 0) requestClose();
	...
	return (
		<Modal open style={{ zIndex: 100000 }} onEscapeKeyDown={requestClose}>
			<div className={style.root} style={{ outline: 'none' }}>
				<div className={style.header} style={{ zIndex: 9999999 }}>
					<div className={style.textContainer}>
						<div className={style.textLarge}>
							Você possui {availableSpecialPacks} pacotes especiais
						</div>
						<div className={style.textSmall} style={{ top: '-6px' }}>
							{availableSpecialPacks > 0 ? 'Clique nele para abrir' : ''}
						</div>
					</div>
				...
				>
					{availableSpecialPacks > -1 && (
						<Content ref={cardRef} onOpen={openPack} onFailure={failure} />
					)}
				</div>
				{isPackOpen && (
