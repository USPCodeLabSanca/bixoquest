  ...
  function handleBackdropClick (event) {
	...
	return (
		<Modal open className={style.root} onClose={handleClose}>
			<div className={style.backdrop} onClick={handleBackdropClick} style={{ backgroundColor: 'rgba(0, 0, 0, 0.5)' }}>
        <div className={style.card} onClick={e => e.stopPropagation()}>
          <div className={style.name}>{memorablePlace.name}</div>
          <Carousel animation='slide' navButtonsAlwaysVisible='true' indicators={false} interval={6000}>
