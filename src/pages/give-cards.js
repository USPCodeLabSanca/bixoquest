...
function QrCode({ selectedCards }) {
	...
	async function fetchToken() {
		...
		try {
			...
			const {
				data: { data: token },
			} = await API.fetchDonationToken({ stickers: cards });
			setToken(token);
		} catch (e) {
			console.error(e);
		} finally {
	...
	function renderCode() {
		...
		if (!token) {
			...
			return <QrCodeGen value={token} size={window.innerWidth - 32} style={codeStyle.code} />;
		}
	}

	return (
		<div className={codeStyle.root}>
			<div className={codeStyle.container}>{renderCode()}</div>
		</div>
	);
}
...
function GiveCards() {
	...
	function renderQrCode() {
		return (
			<div className={codeStyle.root}>
				<div className={codeStyle.title}>
					A pessoa que receberá essa doação deve escanear esse QRCode através da funcionalidade de
					câmera do BixoQuest
				</div>
	...
	return (
		<div className={style.root}>
			...
			<main className={style.main}>{contentRenderer()}</main>
			<footer className={style.footer}>
				<div className={style.footerButtonContainer}>
					{step > 0 && (
						<Button style={style.prevButton} onClick={prevStep} variant="contained">
							Voltar
						</Button>
					)}
				</div>
